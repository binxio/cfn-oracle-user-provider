...
AWS_ACCOUNT_IDS_WITH_READ_ACCESS?=
...
deploy-pipeline:   ## deploy the CI/CD pipeline
	aws cloudformation deploy \
		...
		--parameter-overrides \
			Name=$(NAME) \
			AWSAccountIdsWithReadAccess=$(AWS_ACCOUNT_IDS_WITH_READ_ACCESS) \
			AWSAccountIdsLambdaWithReadAccess=$(shell echo $(AWS_ACCOUNT_IDS_WITH_READ_ACCESS) | sed -e 's/[0-9][0-9]*/arn:aws:lambda:*:&:function:*/g')
